...
func TestMessageRenderCacheAvoidsRepeatWork(t *testing.T) {
	...
	require.True(t, m.messages[0].renderCache.hit(m.messageAreaWidth(), false, len(m.messages[0].text), false, m.messages[0].detailStatus))
}
...
func TestGlamourRenderMsgUpdatesCache(t *testing.T) {
	...
	require.True(t, updated.messages[0].renderCache.hit(m.messageAreaWidth(), false, len(source), false, updated.messages[0].detailStatus))
}
